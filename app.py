...
if run_btn:
    ...
    with st.spinner(f"Downloading data for {ticker_input}..."):
        try:
            ...
            # group_by="ticker" forces a clean column structure in all yfinance versions
            ...
            # Newer yfinance (0.2.x+) returns MultiIndex columns like:
            #   (Price, Ticker) e.g. ("Close", "AAPL")
            # We flatten to a simple single-level DataFrame so ["Close"] works.
            if isinstance(raw.columns, pd.MultiIndex):
                raw.columns = raw.columns.get_level_values(0)

            # Safety check — confirm "Close" column exists after flattening
            if "Close" not in raw.columns:
                st.error(f"'Close' column not found. Columns returned: {list(raw.columns)}")
                st.stop()

            # Extract the closing price as a 1-D Series for calculations
            # .squeeze() converts a single-column DataFrame to a Series if needed
            close = raw["Close"].squeeze()

            # Drop any NaN rows that may appear at the start of the series
            close = close.dropna()

            if len(close) < 50:
                st.warning(f"Only {len(close)} data points returned — some calculations "
                           f"requiring 50+ days (MA50) may be unavailable.")
    ...
    with st.spinner("Downloading portfolio and benchmark data..."):
        try:
            ...
            # Download all tickers at once; progress=False suppresses console noise
            port_full = yf.download(all_tickers, period="1y", auto_adjust=True,
                                    progress=False, group_by="ticker")

            # ── Fix for newer yfinance MultiIndex columns ─────────
            # When multiple tickers are downloaded, yfinance returns a MultiIndex
            # with levels (Price, Ticker) e.g. ("Close", "AAPL").
            # We extract only the "Close" row from the top level.
            if isinstance(port_full.columns, pd.MultiIndex):
                # Stack format: top level = price type, second level = ticker
                port_raw = port_full["Close"]
            else:
                # Single ticker or already flat — just grab Close
                port_raw = port_full[["Close"]] if "Close" in port_full.columns else port_full

            # Ensure port_raw is a DataFrame (not a Series)
            if isinstance(port_raw, pd.Series):
                port_raw = port_raw.to_frame()

            # Drop any tickers that returned no data at all
            port_raw.dropna(axis=1, how="all", inplace=True)
            # Drop rows where ALL prices are NaN (e.g. weekends/holidays at edges)
